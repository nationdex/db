...
exports.default = new forgescript_1.NativeFunction({
    ...
    async execute(ctx, [name, sortType, member, guild]) {
        ...
        const index = data.sort((x, y) => (sortType === util_1.SortType.asc ? Number(x.value) - Number(y.value) : Number(y.value) - Number(x.value))).findIndex((s) => s.id === (member ?? ctx.member?.id));
