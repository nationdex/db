...
exports.default = new forgescript_1.NativeFunction({
    ...
    async execute(ctx, [name, channel, def]) {
        const data = await util_1.DataBase.get({ name, id: channel?.id ?? ctx.channel.id, type: "channel", guildId: channel?.guild.id ?? ctx.guild?.id }).then((x) => x?.value);
...
//# sourceMappingURL=getChannelVar.js.map